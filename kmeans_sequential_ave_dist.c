...
void ave_dist(float **objects,      /* in: [numObjs][dimentions] */
                   int     dimentions,    /* no. features */
                   int     numObjs,      /* no. objects */
                   int     numClusters,
                   int    *membership,
                   float **clusters )
{
      ...
      for(int i=0; i<numObjs; i++){
        temp=euclidian_distance(dimentions,objects[i],clusters[membership[i]]);
        temp=sqrt(temp);
        dist+=temp;
      }
...
int main(int argc, char **argv) {
    ...
    ave_dist(objects,dimentions, numObjs, numClusters, membership, clusters);
